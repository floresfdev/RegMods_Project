...
In the model, `{r coeffPrimary[1]` is the mean of MPG for the automatic transmission. For the case of manual transmission, the value of `{r coeffPrimary[2]}` is an additional increase.
...
By looking at the R-squared value we can see that the transmission type explains only `{r round(rSquaredPrimary, 2) * 100}`% of the variability of MPG. This value is not good enough for our analysis, so we will have to use nested models to select a better one.
