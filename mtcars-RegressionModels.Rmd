...
According to the p-value, which is `r round(coeffPrimary[8], 5)` and <0.05, this model is statistically significant.
...
The full summary of coefficients can be found in appendix A5. Because this are multivariable models, the adjusted R-squared is a better metric to evaluate variability. In the second model, the transmission type explains `r round(adjRSquared2, 2) * 100`% of the variability in MPG, while the third explains `r round(adjRSquared3, 2) * 100`% and it's still parsimonious, even more than the previous one.

**Therefore, the linear model which fits transmission type, horsepower and weight as regressors, is a better model to predict MPG.**
...
Plots of this features can be found in appendix A6. Regarding potencial influence and leverage points, the tables can be found in appendix A7.
...
The point for `Maserati Bora` has high potential for leverage but no influence.